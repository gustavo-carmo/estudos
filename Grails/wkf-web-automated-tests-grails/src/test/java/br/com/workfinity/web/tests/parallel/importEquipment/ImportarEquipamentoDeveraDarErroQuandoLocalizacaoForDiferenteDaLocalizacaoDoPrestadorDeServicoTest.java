...
public class ImportarEquipamentoDeveraDarErroQuandoLocalizacaoForDiferenteDaLocalizacaoDoPrestadorDeServicoTest
		extends TestBase {

	private static final String USER_IMPORT_EQUIP_LOCATION_DIFFERENT_OF_SERVICE_PROVIDER = "USER_LOCATION_DIF_SP_"
			+ randomString(5);
	...
	public void background() {
		...
		createUserAndDoLogin(USER_IMPORT_EQUIP_LOCATION_DIFFERENT_OF_SERVICE_PROVIDER, "123456",
				Arrays.asList("ROLE_IMPORT_PROCESS", "ROLE_IMPORT_PROCESS_FROM_TO"), "pt_BR");
	}

	@Test(dependsOnMethods = { "background" })
	public void deveraDarErroQuandoLocalizacaoNaoForDeAcordoComALocalizacaoDoPrestadorDeServico() {
		...
		String status = importProcessShowPage.getStatus("status10");
		importProcessShowPage.assertEquals("Erro", status, "Diferentes!");

		// Verificando o detalhe do erro
		String details = importProcessShowPage.getDetails("val10");
		importProcessShowPage.assertEquals("Location: Outro_Prestador not found.", details, "Diferentes!");
	}
}
