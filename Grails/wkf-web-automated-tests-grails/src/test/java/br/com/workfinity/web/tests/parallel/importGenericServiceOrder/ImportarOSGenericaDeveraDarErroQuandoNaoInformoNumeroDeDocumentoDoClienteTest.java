...
public class ImportarOSGenericaDeveraDarErroQuandoNaoInformoNumeroDeDocumentoDoClienteTest extends TestBase {

	private static final String USER_IMPORT_SERVICE_ORDER_GENERIC_WITHOUT_DOCUMENT_NUMBER = "USER_ISOG_SEM_DOC_NUM_"
			+ randomString(5);
	...
	public void background() {
		...
		createUserAndDoLogin(USER_IMPORT_SERVICE_ORDER_GENERIC_WITHOUT_DOCUMENT_NUMBER, "123456",
				Arrays.asList("ROLE_IMPORT_PROCESS", "ROLE_IMPORT_PROCESS_FROM_TO"), "pt_BR");
	}

	@Test(dependsOnMethods = { "background" })
	public void deveraDarErroQuandoNaoInformoOCampoCustomerDocumentNumber() {
		...
		String status = importProcessShowPage.getStatus("status36");
		importProcessShowPage.assertEquals("Erro", status, "Diferentes!");

		// Verificando o detalhe do erro
		String details = importProcessShowPage.getDetails("val36");
		importProcessShowPage.assertEquals("Cliente: Campo \"Documento\" é requerido", details, "Diferentes!");
	}
}
