...
public class ImportarOSGenericaDeveraDarErroQuandoInformoUmStatusInexistenteTest extends TestBase {

	private static final String USER_IMPORT_SERVICE_ORDER_GENERIC_STATUS_NONEXISTENT = "USER_ISOG_STATUS_NONE_"
			+ randomString(5);
	...
	public void background() {
		...
		createUserAndDoLogin(USER_IMPORT_SERVICE_ORDER_GENERIC_STATUS_NONEXISTENT, "123456",
				Arrays.asList("ROLE_IMPORT_PROCESS", "ROLE_IMPORT_PROCESS_FROM_TO"), "pt_BR");
	}

	@Test(dependsOnMethods = { "background" })
	public void deveraDarErroQuandoNoCampoStatusNameInformoUmStatusInexistente() {
		...
		String status = importProcessShowPage.getStatus("status29");
		importProcessShowPage.assertEquals("Erro", status, "Diferentes!");

		// Verificando o detalhe do erro
		String details = importProcessShowPage.getDetails("val29");
		importProcessShowPage.assertEquals("Ordem de Serviço: Campo \"Status\" é requerido.", details, "Diferentes!");
	}
}
