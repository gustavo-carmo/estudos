...
public class ImportarOSGenericaDeveraDarErroQuandoInformoApelidoDePrestadorDeServicoInexistenteTest extends TestBase {

	private static final String USER_IMPORT_SERVICE_ORDER_GENERIC_SERVICE_PROVIDER_NONEXISTENT = "USER_ISOG_SP_NONE_"
			+ randomString(5);
	...
	public void background() {
		...
		createUserAndDoLogin(USER_IMPORT_SERVICE_ORDER_GENERIC_SERVICE_PROVIDER_NONEXISTENT, "123456",
				Arrays.asList("ROLE_IMPORT_PROCESS", "ROLE_IMPORT_PROCESS_FROM_TO"), "pt_BR");
	}

	// TODO - verificar porque aceitou um prestador de serviço inexistente
	@Test(dependsOnMethods = { "background" })
	public void deveraDarErroQuandoNoCampoServiceProviderAliasInformoUmServiceProviderAliasInexistente() {
		...
		String status = importProcessShowPage.getStatus("status26");
		importProcessShowPage.assertEquals("Erro", status, "Diferentes!");

		// Verificando o detalhe do erro
		String details = importProcessShowPage.getDetails("val26");
