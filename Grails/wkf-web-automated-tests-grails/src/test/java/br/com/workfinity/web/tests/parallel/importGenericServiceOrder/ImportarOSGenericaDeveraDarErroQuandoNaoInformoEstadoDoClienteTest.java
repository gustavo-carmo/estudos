...
public class ImportarOSGenericaDeveraDarErroQuandoNaoInformoEstadoDoClienteTest extends TestBase {

	private static final String USER_IMPORT_SERVICE_ORDER_GENERIC_WITHOUT_CUSTOMER_STATE = "USER_ISOG_SEM_CL_STATE_"
			+ randomString(5);
	...
	public void background() {
		...
		createUserAndDoLogin(USER_IMPORT_SERVICE_ORDER_GENERIC_WITHOUT_CUSTOMER_STATE, "123456",
				Arrays.asList("ROLE_IMPORT_PROCESS", "ROLE_IMPORT_PROCESS_FROM_TO"), "pt_BR");
	}

	@Test(dependsOnMethods = { "background" })
	public void deveraDarErroQuandoNaoInformoOCampoCustomerAddressState() {
		...
		String status = importProcessShowPage.getStatus("status32");
		importProcessShowPage.assertEquals("Erro", status, "Diferentes!");

		// Verificando o detalhe do erro
		String details = importProcessShowPage.getDetails("val32");
