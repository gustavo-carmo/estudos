...
public class ImportarOSGenericaDeveraDarErroQuandoNaoInformoServicoTest extends TestBase {

	private static final String USER_IMPORT_SERVICE_ORDER_GENERIC_WITHOUT_SERVICE = "USER_ISOG_SEM_SERVICE_"
			+ randomString(5);
	...
	public void background() {
		...
		createUserAndDoLogin(USER_IMPORT_SERVICE_ORDER_GENERIC_WITHOUT_SERVICE, "123456",
				Arrays.asList("ROLE_IMPORT_PROCESS", "ROLE_IMPORT_PROCESS_FROM_TO"), "pt_BR");
	}

	@Test(dependsOnMethods = { "background" })
	public void deveraDarErroQuandoNaoInformoOCampoServiceName() {
		...
		String status = importProcessShowPage.getStatus("status41");
		importProcessShowPage.assertEquals("Erro", status, "Diferentes!");

		// Verificando o detalhe do erro
		String details = importProcessShowPage.getDetails("val41");
		importProcessShowPage.assertEquals("For input string: \"null\"", details, "Diferentes!");
	}
}
