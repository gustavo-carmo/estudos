...
public class ShipmentOrderAllowedExistingProductTest extends TestBase {
	...
	@Test
	public void backgroundOrdemDeRemessaProdutoExistente() {
		...
		LoadDataHelper.createEquipment(getDriver(), getBaseUrl(), CONTRACTOR_GENERIC, "", "", "", MANUFACTORE_GENERIC,
				MODEL_GENERIC_PRODUCT, "GOOD", serialNumberProduct, null, false);

		createUserAndDoLogin(USER_SHIP, "123456", ROLES);
	}

	@Test(dependsOnMethods = { "backgroundOrdemDeRemessaProdutoExistente" })
	...
	public void falhaAoAdicionarUmProdutoNovoNaOrdemDeRemessa() {
		...
		shipmentOrderAddEquipment.validateMessageError("Produtos novos não são permitidos");
//		shipmentOrderAddEquipment.validateMessageErrorSimple("Produtos novos não são permitidos");
		...
		shipmentOrderAddEquipment.validateIfContainsTheRowInTableByCssSelector(
				Arrays.asList(CONTRACTOR_GENERIC, MODEL_GENERIC_PRODUCT, serialNumberRandom,
						"Produtos novos não são permitidos"),
				"#collapseThree .table.table-bordered.table-striped.table-condensed > tbody > tr",
				"Erro na validação da tabela de erro");
	}

	@Test(dependsOnMethods = { "falhaAoAdicionarUmProdutoNovoNaOrdemDeRemessa" })
	public void sucessoAoAdicionarUmProdutoExistenteNaOrdemDeRemessa() {
