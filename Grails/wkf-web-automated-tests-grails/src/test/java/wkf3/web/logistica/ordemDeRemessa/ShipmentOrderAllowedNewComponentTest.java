...
public class ShipmentOrderAllowedNewComponentTest extends TestBase {
	...
	private static final String MODEL_SHIP_COMPONENT = "MODEL_SHIP_NEW_COMPONENT_" + randomString(5);
	...
	@Test
	public void backgroundOrdemDeRemessaNovoComponente() {
		...
		LoadDataHelper.createEquipment(getDriver(), getBaseUrl(), CONTRACTOR_GENERIC, "", "", "", MANUFACTORE_GENERIC,
				MODEL_SHIP_COMPONENT, "GOOD", "", 500, false);

		createUserAndDoLogin(USER_SHIP, "123456", ROLES);
	}

	@Test(dependsOnMethods = { "backgroundOrdemDeRemessaNovoComponente" })
	...
	public void falhaAoAdicionarUmComponenteExistenteNaOrdemDeRemessa() {
		...
		addAccessoryComponentOrSupply.validateMessageError("Componentes existentes não são permitidos");

		addAccessoryComponentOrSupply.validateIfContainsTheRowInTableByCssSelector(
				Arrays.asList(MODEL_SHIP_COMPONENT, "Componentes existentes não são permitidos"),
				"#collapseThree .table.table-bordered.table-striped.table-condensed > tbody > tr",
				"Erro na validação da tabela de erro");
	}

	@Test(dependsOnMethods = { "falhaAoAdicionarUmComponenteExistenteNaOrdemDeRemessa" })
	public void sucessoAoAdicionarUmComponenteNovoNaOrdemDeRemessa() {

		addAccessoryComponentOrSupply.setSituation("Bad");
		addAccessoryComponentOrSupply.setQuantity("300");
		addAccessoryComponentOrSupply.buttonAddComponents();

		addAccessoryComponentOrSupply.validateIfContainsTheRowInTableByCssSelector(
				Arrays.asList(MODEL_SHIP_COMPONENT, "300", "Novo equipamento adicionado."),
