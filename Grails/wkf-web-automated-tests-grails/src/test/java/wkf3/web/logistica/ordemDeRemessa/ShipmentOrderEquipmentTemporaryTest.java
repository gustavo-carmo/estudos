...
public class ShipmentOrderEquipmentTemporaryTest extends TestBase {
	...
	@Test
	public void backgroundOrdemDeRemessaEquipamentoTemporario() {

		createUserAndDoLogin(USER_SHIP, "123456", ROLES);
	}

	@Test(dependsOnMethods = { "backgroundOrdemDeRemessaEquipamentoTemporario" })
	...
	public void verificandoSeOEquipamentoTemporarioEstaNoEstoque() {
		...
		visitStockControl.validateIfContainsTheRowInTable(Arrays.asList(CONTRACTOR_GENERIC, serialNumberProduct, "Ordem de Remessa: " + codeShipmentOrder));
	}
	...
	public void verificandoSeOEquipamentoNaoEMaisTemporario() {

		visitStockControl = navBar.visitStockControl();
		visitStockControl.setSerialNumber(serialNumberProduct);
		visitStockControl.setCheckBoxTemporary(false);
		visitStockControl.buttonSearch();

		visitStockControl.validateIfContainsTheRowInTable(
				Arrays.asList(CONTRACTOR_GENERIC, SERVICE_PROVIDER_GENERIC_MATRIZ, serialNumberProduct));
		
		EquipmentShowPage equipmentShowPage = visitStockControl.showItemTable(1);
		
		String temporaryText = equipmentShowPage.getTemporaryText();
		
		equipmentShowPage.assertEquals(temporaryText, "Não", "Valores diferentes");
	}
}
