...
public class TriggerAssociaEquipamentoAoClienteTest extends TestBase {

	private static final String USER_TRIGGER_ASSOCIATE_EQUIP_CUSTOMER = "USER_TR_ASS_EQ_CL_" + randomString(5);
	...
	@Test
	public void loadDataTriggerAssociaEquipamentoAoCliente() {
		...
		createUserAndDoLogin(USER_TRIGGER_ASSOCIATE_EQUIP_CUSTOMER, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataTriggerAssociaEquipamentoAoCliente" })
	public void deveraAcionarOEventoDaTriggerQuandoATriggerDeAssociaOEquipamentoAoClienteEstaHabilitada() {
		...
		serviceOrderCrudForm.setStatus(TRIGGER_ASSOCIATE_EQUIPMENT_TO_SERVICE_PROVIDER);
		serviceOrderCrudForm.buttonUpdate();
		...
		stockControlListPage
				.validateIfContainsTheRowInTable(Arrays.asList(serialNumberEquipment, SERVICE_PROVIDER_GENERIC_MATRIZ));

		// Voltando na Ordem de Serviço para Associar o Equipamento ao Cliente
		// com a Trigger
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);

		serviceOrderCrudForm.setStatus(TRIGGER_ASSOCIATE_EQUIPMENT_TO_CUSTOMER);
		serviceOrderCrudForm.buttonUpdate();
