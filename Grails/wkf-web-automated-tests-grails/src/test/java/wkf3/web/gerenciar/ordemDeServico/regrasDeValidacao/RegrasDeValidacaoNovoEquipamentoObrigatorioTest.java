...
public class RegrasDeValidacaoNovoEquipamentoObrigatorioTest extends TestBase {

	private static final String USER_VR_NEW_EQUIP_REQUIRED = "USER_VR_NEW_EQUIP_REQ_" + randomString(5);
	...
	@Test
	public void loadDataRegrasDeValidacaoNovoEquipamentoRequerido() {
		...
		createUserAndDoLogin(USER_VR_NEW_EQUIP_REQUIRED, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataRegrasDeValidacaoNovoEquipamentoRequerido" })
	public void deveraDarErroQuandoARegraDeValidacaoDeNovoEquipamentoObrigatorioNaoForAtendida() {
		...
		serviceOrderCrudForm.setStatus(VALIDATION_RULE_NEW_EQUIP_REQUIRED);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm.validateErrorMessage("Novo Equipamento é campo obrigatório! (Validação do próximo passo)");
	}

	@Test(dependsOnMethods = { "deveraDarErroQuandoARegraDeValidacaoDeNovoEquipamentoObrigatorioNaoForAtendida" })
	public void deveraMudarComSucessoOStatusDeUmaOrdemDeServicoQuandoARegraDeValidacaoDeNovoEquipamentoObrigatorioForAtendida() {
