...
public class MesclarEquipamentoNaOrdemDeServicoEstoqueClienteTest extends TestBase {

	private static final String USER_MERGE_EQUIP_STOCK_CL = "USER_MEQ_STOCK_CL_" + randomString(5);
	...
	@Test
	public void loadDataMesclarEquipamentoNaOrdemDeServicoEstoqueDoCliente() {

		// Equipamento no Estoque do Cliente
		...
		LoadDataHelper.createServiceOrder(getDriver(), getBaseUrl(), "CNPJ", CUSTOMER_DOCUMENT_NUMBER,
				SERVICE_MERGE_EQUIP_STOCK_CUSTOMER);

		// Pegando o Id da Ordem de Serviço no Console
		String idServiceOrder = getDriver().findElement(By.tagName("body")).getText();

		createUserAndDoLogin(USER_MERGE_EQUIP_STOCK_CL, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataMesclarEquipamentoNaOrdemDeServicoEstoqueDoCliente" })
	public void deveraMesclarUmEquipamentoDoEstoqueDoClienteComSucessoNaOrdemDeServicoQuandoNoServicoDoContrantateTemAFlagUsarEstoqueDoClienteEstiverHabilitada() {
		...
		// Fazendo a Mesclagem do Equipamento
