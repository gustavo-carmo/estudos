...
public class TriggerConverteEquipamentoParaSituacaoGoodTest extends TestBase {

	private static final String USER_TRIGGER_CHANGE_EQUIPMENT_TO_GOOD = "USER_TR_CHANGE_GOOD_" + randomString(5);
	...
	@Test
	public void loadDataTriggerConventerEquipamentoParaSituacaoGood() {
		...
		createUserAndDoLogin(USER_TRIGGER_CHANGE_EQUIPMENT_TO_GOOD, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataTriggerConventerEquipamentoParaSituacaoGood" })
	public void deveraAcionarOEventoDaTriggerQuandoATriggerDeConverteOEquipamentoParaSituacaoGOODEstaHabilitada() {
		...
		serviceOrderCrudForm.setStatus(TRIGGER_CHANGE_EQUIPMENT_TO_BAD);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm.validateMessageSuccessUpdate();

		// Verificando no Estoque se o Equipamento esta com Situação Bad
		...
		stockControlListPage.validateIfContainsTheRowInTable(Arrays.asList(serialNumberEquipment, "Bad"));

		// Voltando para a Ordem de Serviço para Mudar a Situação do Equipamento
		// para Good
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);

		serviceOrderCrudForm.setStatus(TRIGGER_CHANGE_EQUIPMENT_TO_GOOD);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm.validateMessageSuccessUpdate();

		// Verificando no Estoque se o Equipamento esta com Situação Good
		stockControlListPage = navBar.visitStockControl();
		stockControlListPage.setSerialNumber(serialNumberEquipment);
		stockControlListPage.buttonSearch();

		stockControlListPage.validateIfContainsTheRowInTable(Arrays.asList(serialNumberEquipment, "Good"));
	}
}
