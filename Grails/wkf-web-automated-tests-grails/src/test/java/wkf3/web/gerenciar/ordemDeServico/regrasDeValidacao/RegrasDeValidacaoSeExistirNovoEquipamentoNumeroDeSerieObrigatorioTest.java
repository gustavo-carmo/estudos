...
public class RegrasDeValidacaoSeExistirNovoEquipamentoNumeroDeSerieObrigatorioTest extends TestBase {

	private static final String USER_VR_IF_NEW_EQUIP_SERIAL_NUMBER_REQUERID = "USER_VR_IF_NEQ_SERIAL_REQ_"
			+ randomString(5);
	...
	@Test
	public void loadDataRegrasDeValidacaoSeHouverNovoEquipamentoNumeroDeSerieObrigatorio() {
		...
		createUserAndDoLogin(USER_VR_IF_NEW_EQUIP_SERIAL_NUMBER_REQUERID, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataRegrasDeValidacaoSeHouverNovoEquipamentoNumeroDeSerieObrigatorio" })
	public void deveraDarErroquandoHouverUmNovoEquipamentoEARegraDeValidacaoDeSeExistirNovoEquipamentoONumeroDeSerieEObrigatorioNaoForAtendida() {
		...
		serviceOrderCrudForm.setStatus(VALIDATION_RULE_IF_NEW_EQUIP_SERIAL_NUMBER_REQUIRED);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm.validateErrorMessage(
				"Se existir Novo Equipamento, Nº Série é obrigatório (Validação do Próximo passo)");
	}

	@Test(dependsOnMethods = {
			"deveraDarErroquandoHouverUmNovoEquipamentoEARegraDeValidacaoDeSeExistirNovoEquipamentoONumeroDeSerieEObrigatorioNaoForAtendida" })
	public void deveraMudarComSucessoOStatusDeUmaOrdemDeServicoQuandoHouverUmNovoEquipamentoEARegraDeValidacaoDeSeExistirNovoEquipamentoONumeroDeSerieEObrigatorioForAtendida() {
		...
		// Adicionando Novo Equipamento com Número de Série
		serviceOrderModalEquipmentFind = serviceOrderCrudForm.clickSearchNewEquipment();
		serviceOrderModalEquipmentCreateAndEdit = serviceOrderModalEquipmentFind.buttonNew();
