...
public class TriggerMarcaNovoEquipamentoComoInventariadoTest extends TestBase {

	private static final String USER_TRIGGER_TURN_NEW_EQUIPMENT_INVENTORIED = "USER_TR_TURN_NEQ_INV_" + randomString(5);
	...
	@Test
	public void loadDataTriggerMarcaNovoEquipamentoComoInventariado() {
		...
		createUserAndDoLogin(USER_TRIGGER_TURN_NEW_EQUIPMENT_INVENTORIED, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataTriggerMarcaNovoEquipamentoComoInventariado" })
	public void deveraAcionarOEventoDaTriggerQuandoATriggerDeMarcaONovoEquipamentoComoInventariadoEstaHabilitada() {
		...
		// Adicionando o Novo Equipamento
		...
		serviceOrderCrudForm.setStatus(TRIGGER_TURN_NEW_EQUIPMENT_INVENTORIED);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm.validateMessageSuccessUpdate();

		// Verificando no Estoque se o Equipamento esta Inventáriado
		...
		stockControlListPage.validateIfContainsTheRowInTable(Arrays.asList(serialNumberNewEquipment));

		equipmentShowPage = stockControlListPage.showItemTable(1);

		String inventoried = equipmentShowPage.getInventoriedText();
		equipmentShowPage.assertEquals("Sim", inventoried, "Não esta Inventáriado");
	}
}
