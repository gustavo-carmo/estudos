...
public class RegrasDeValidacaoEquipamentoInventariadoObrigatorioTest extends TestBase {

	private static final String USER_VR_EQUIP_INVENTORIED = "USER_VR_EQUIP_INV_" + randomString(5);
	...
	private String serialNumberEquipmentInventoried;
	private ServiceOrderCrudForm serviceOrderCrudForm;
	private String serialNumberEquipmentNotInventoried;
	...
	@Test
	public void loadDataRegrasDeValidacaoEquipamentoInventariadoObrigatorio() {

		serialNumberEquipmentInventoried = generateLengthNumber(8);

		// Criando o Equipamento Inventariado
		LoadDataHelper.createEquipment(getDriver(), getBaseUrl(), "", "CNPJ", CUSTOMER_DOCUMENT_NUMBER, "",
				MANUFACTORE_GENERIC, MODEL_GENERIC_PRODUCT, "GOOD", serialNumberEquipmentInventoried, null, true);

		serialNumberEquipmentNotInventoried = generateLengthNumber(8);

		// Criando o Equipamento não Inventariado
		LoadDataHelper.createEquipment(getDriver(), getBaseUrl(), "", "CNPJ", CUSTOMER_DOCUMENT_NUMBER, "",
				MANUFACTORE_GENERIC, MODEL_GENERIC_PRODUCT, "GOOD", serialNumberEquipmentNotInventoried, null, false);
		...
		createUserAndDoLogin(USER_VR_EQUIP_INVENTORIED, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataRegrasDeValidacaoEquipamentoInventariadoObrigatorio" })
	public void deveraDarErroQuandoARegraDeValidacaoDeEquipamentoInventariadoObrigatorioNaoForAtendida() {

		// Adicionando O Equipamento Não Inventáriado
		serviceOrderModalEquipmentFind = serviceOrderCrudForm.clickSearchEquipment();
		serviceOrderModalEquipmentFind.setSerialNumber(serialNumberEquipmentNotInventoried);

		serviceOrderModalEquipmentFind
				.validateIfContainsTheRowInTable(Arrays.asList(serialNumberEquipmentNotInventoried));
		...
		serviceOrderCrudForm.setStatus(VALIDATION_RULE_EQUIP_INVENTORIED_REQUIRED);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm.validateErrorMessage("O Equipamento deve ser Inventariado! (Validação do Próximo passo)");
	}

	@Test(dependsOnMethods = {
			"deveraDarErroQuandoARegraDeValidacaoDeEquipamentoInventariadoObrigatorioNaoForAtendida" })
	public void deveraMudarComSucessoOStatusDeUmaOrdemDeServicoQuandoARegraDeValidacaoDeEquipamentoInventariadoObrigatorioForAtendida() {

		serviceOrderCrudForm.buttonRemovedEquipment();

		// Adicionando o Equipamento Inventáriado
		serviceOrderModalEquipmentFind = serviceOrderCrudForm.clickSearchEquipment();
		serviceOrderModalEquipmentFind.setSerialNumber(serialNumberEquipmentInventoried);

		serviceOrderModalEquipmentFind.validateIfContainsTheRowInTable(Arrays.asList(serialNumberEquipmentInventoried));
