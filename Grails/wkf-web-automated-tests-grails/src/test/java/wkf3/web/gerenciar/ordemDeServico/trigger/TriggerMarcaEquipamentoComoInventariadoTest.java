...
public class TriggerMarcaEquipamentoComoInventariadoTest extends TestBase {

	private static final String USER_TRIGGER_TURN_EQUIPMENT_INVENTORIED = "USER_TR_TURN_EQ_INV_" + randomString(5);
	...
	@Test
	public void loadDataTriggerMarcaEquipamentoComoInventariado() {
		...
		createUserAndDoLogin(USER_TRIGGER_TURN_EQUIPMENT_INVENTORIED, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataTriggerMarcaEquipamentoComoInventariado" })
	public void deveraAcionarOEventoDaTriggerQuandoATriggerDeMarcaOEquipamentoComoInventariadoEstaHabilitada() {
		...
		serviceOrderCrudForm.setStatus(TRIGGER_TURN_EQUIPMENT_INVENTORIED);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm.validateMessageSuccessUpdate();

		// Verificando no Estoque se o Equipamento esta Inventariado
		...
		stockControlListPage.validateIfContainsTheRowInTable(Arrays.asList(serialNumberEquipment));
