...
public class RegrasDeValidacaoEquipamentoNumeroLogicoObrigatorioTest extends TestBase {

	private static final String USER_VR_EQUIP_LOGIC_NUMBER = "USER_VR_EQ_LOGIC_" + randomString(5);
	...
	@Test
	public void loadDataRegrasDeValidacaoEquipamentoComNumeroLogicoObrigatorio() {
		...
		createUserAndDoLogin(USER_VR_EQUIP_LOGIC_NUMBER, "123456", ROLES, "pt_BR");

		// Acessando a Ordem de Serviço
		serviceOrderCrudForm = new ServiceOrderCrudForm(getDriver(), getBaseUrl(), idServiceOrder);
	}

	@Test(dependsOnMethods = { "loadDataRegrasDeValidacaoEquipamentoComNumeroLogicoObrigatorio" })
	public void deveraDarErroQuandoARegraDeValidacaoDeNumeroLogicoObrigatorioNaoForAtendida() {

		// Adicionando um Equipamento Sem Número Lógico
		...
		serviceOrderCrudForm.setStatus(VALIDATION_RULE_EQUIP_LOGIC_NUMBER_REQUIRED);
		serviceOrderCrudForm.buttonUpdate();

		serviceOrderCrudForm
				.validateErrorMessage("O Número Lógico do Equipamento é requerido! (Validação do Próximo passo)");
	}

	@Test(dependsOnMethods = { "deveraDarErroQuandoARegraDeValidacaoDeNumeroLogicoObrigatorioNaoForAtendida" })
	public void deveraMudarComSucessoOStatusDeUmaOrdemDeServicoQuandoARegraDeValidacaoDeNumeroLogicoObrigatorioForAtendida() {

		serviceOrderCrudForm.buttonRemovedEquipment();

		// Armezando um número lógico na variavel
		String logicNumberEquipment = generateLengthNumber(8);

		// Adicionando Equipamento com Número Lógico
		serviceOrderModalEquipmentFind = serviceOrderCrudForm.clickSearchEquipment();
		serviceOrderModalEquipmentCreateAndEdit = serviceOrderModalEquipmentFind.buttonNew();

		serviceOrderModalEquipmentCreateAndEdit.setManufacturer(MANUFACTORE_GENERIC);
		serviceOrderModalEquipmentCreateAndEdit.setModel(MODEL_GENERIC_PRODUCT);
		serviceOrderModalEquipmentCreateAndEdit.setContractorExternalId(logicNumberEquipment);
		serviceOrderModalEquipmentCreateAndEdit.buttonSave();
